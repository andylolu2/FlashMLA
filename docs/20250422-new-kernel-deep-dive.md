...
1. [0] Compute $`\vec p_0 = \vec q K_0^\intercal / qk_{scale}`$.
2. [1] Compute $`\vec p_1 = \vec q K_1^\intercal / qk_{scale}`$.
3. [0] Compute $mp_0 = \max(\vec p_0)$, $`m_{new_0} = \max(m, mp_0)`$, and $`scale_0 = \exp(m_{new_0} - m)`$. Update $`m \gets m_{new_0}`$.
4. [0] Perform softmax on $\vec p_0$: $`\vec p_0 \gets \exp(\vec p_0 - m_{new_0})`$.
5. [0] Update $\vec o_L \gets \vec o_L \cdot scale_0 + \vec p_0 V_{0L}$.
6. [1] Compute $mp_1 = \max(\vec p_1)$, $`m_{new_1} = \max(m, mp_1)`$, and $`scale_1 = \exp(m_{new_1} - m)`$. Update $`m \gets m_{new_1}`$.
7. [1] Perform softmax on $\vec p_1$: $`\vec p_1 \gets \exp(\vec p_1 - m_{new_1})`$.
